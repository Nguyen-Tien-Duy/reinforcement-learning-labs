...
```
Dự án của bạn là một sự kết hợp rất thú vị giữa **Blockchain (EIP-1559)**, **Offline RL** và **Safety Engineering**. Để hiểu sâu và bảo vệ được luận điểm "Hybrid Architecture" của mình, bạn nên đọc các paper "kinh điển" chia theo 3 nhóm sau:

### 1. Nhóm Blockchain & Gas Mechanism (Hiểu về "Vật lý" của hệ thống)
Nhóm này giúp bạn hiểu tại sao `target_gas` và `base_fee` lại biến động như vậy.

*   **Paper quan trọng nhất:** *"Transaction Fee Mechanism Design with Analysis of EIP-1559"* - **Tim Roughgarden (2021)**. 
    *   *Tại sao cần đọc:* Đây là "kinh thánh" về EIP-1559. Nó giải thích logic toán học đằng sau việc tại sao giá gas lại tăng/giảm dựa trên utilization — thứ mà bạn đang mô phỏng trong `src/physics.py`.
*   **Paper bổ trợ:** *"Dynamic Gas Pricing and its Impact on Ethereum Transaction Performance"* - (Nghiên cứu về độ trễ và mempool).

### 2. Nhóm Offline RL Algorithms (Hiểu về các "Brain" bạn đang dùng)
Trong code bạn đang dùng CQL, IQL, TD3+BC. Bạn cần đọc để hiểu tại sao Offline RL lại khó hơn Online RL.

*   **CQL (Conservative Q-Learning):** *"Conservative Q-Learning for Offline Reinforcement Learning"* - **Kumar et al. (2020)**. 
    *   *Key point:* Hiểu tại sao phải "thận trọng" (conservative) để không bị ảo tưởng về các hành động không có trong dữ liệu (Out-of-Distribution).
*   **IQL (Implicit Q-Learning):** *"Offline Reinforcement Learning with Implicit Q-Learning"* - **Kostrikov et al. (2021)**.
    *   *Key point:* Cách học mà không cần phải thực hiện `max Q` trên các hành động lạ, rất phù hợp với dữ liệu Gas vốn nhiều nhiễu.
*   **TD3+BC:** *"A Minimalist Approach to Offline Reinforcement Learning"* - **Fujimoto et al. (2021)**.
    *   *Key point:* Cách kết hợp đơn giản giữa Behavioral Cloning và RL để giữ Agent không đi quá xa khỏi dữ liệu lịch sử.

### 3. Nhóm Safe RL & Shielding (Hiểu về cái "Khiên" bạn đang định làm)
Đây là phần giúp dự án của bạn khác biệt so với các bài toán RL thông thường.

*   **Shielding Theory:** *"Safe Reinforcement Learning via Shielding"* - **Alshiekh et al. (2018)**.
    *   *Tại sao cần đọc:* Đây chính là cơ sở lý thuyết cho cái **Layer 2** mà bạn viết trong docs. Nó dạy cách "cướp quyền" Agent khi sắp có nguy hiểm.
*   **Constrained RL:** *"Constrained Policy Optimization"* - **Achiam et al. (2017)**.
    *   *Key point:* Hiểu về Lagrangian multipliers — thứ mà bạn dùng để biến các ràng buộc (deadline) thành điểm phạt (penalty) trong reward.

### Lời khuyên "Chiến thuật":
*   Nếu bạn đang vội, hãy đọc kỹ **Paper của Tim Roughgarden** (để hiểu bài toán) và **Paper về Shielding** (để hiểu giải pháp an toàn). 
*   Các paper về thuật toán (CQL, IQL) bạn chỉ cần nắm được ý tưởng chính là chúng giải quyết vấn đề "Overestimation" trong dữ liệu Offline như thế nào.