...
# Load model
algo = d3rlpy.load_learnable("/mnt/WindowsD/Reinforcement Learning/labs/Final_Project/output/toy_iql_20260411_185317.d3", device="cpu")
...
print("Testing first 10 steps:")
for i in range(10):
    action = int(algo.predict(np.array([state]))[0])
    print(f"Step {i}: raw_q={env.queue_size:.1f}, time={env.time_to_deadline:.1f}, state={state}, action={action}")
    state, r, term, trunc, i = env.step(action)
