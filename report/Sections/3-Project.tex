...
\textit{(Phần này sẽ được cập nhật sau khi quá trình huấn luyện 400,000 steps hoàn tất. Các số liệu interim từ 50k steps đầu được trình bày ở đây.)}

\subsection{Đường cong Huấn luyện}

\begin{table}[ht]
\centering
\caption{Diễn biến loss theo epoch — Discrete CQL V6}
\begin{tabular}{cccc}
\hline
\textbf{Epoch} & \textbf{Steps} & \textbf{td\_loss (Bellman Error)} & \textbf{conservative\_loss} \\
\hline
1  & 10,000  & 0.0754 & 1.010 \\
2  & 20,000  & 0.0774 & 0.937 \\
3  & 30,000  & 0.0776 & 0.922 \\
5  & 50,000  & 0.0793 & 0.904 \\
8  & 80,000  & 0.0793 & 0.890 \\
10 & 100,000 & 0.0790 & 0.886 \\
13 & 130,000 & 0.0804 & 0.881 \\
\hline
\end{tabular}
\label{tab:training_curve}
\end{table}

\textbf{Nhận xét:} \texttt{conservative\_loss} giảm đều từ 1.01 xuống 0.881 qua 130k steps — Q-network đang học phân biệt in-distribution (Oracle) actions với OOD actions. \texttt{td\_loss} ổn định $\approx 0.078$ — Bellman equation được thỏa mãn nhất quán.

\subsection{So sánh với IQL Continuous}

\begin{table}[ht]
\centering
\caption{Leaderboard Evaluation — 7 episodes holdout}
\label{tab:eval}
\begin{tabular}{lcc}
\hline
\textbf{Model / Chiến lược} & \textbf{Miss Rate} & \textbf{Cost / Episode} \\
\hline
IQL Continuous V4 (tất cả checkpoint) & 1.000 & $\approx$726,300 \\
\hline
Discrete CQL V6 -- 10k steps & 1.000 & 349,414 \\
Discrete CQL V6 -- 20k steps & 1.000 & 315,375 \\
\textbf{Discrete CQL V6 -- 30k steps} & \textbf{0.857} & \textbf{286,391} \\
Discrete CQL V6 -- 40k steps & 1.000 & 258,026 \\
Discrete CQL V6 -- 50k steps & 1.000 & 257,791 \\
\hline
\end{tabular}
\end{table}
...
\textbf{Phân tích:}
\begin{itemize}
    \item \textbf{IQL V4}: Policy collapse hoàn toàn — 100\% miss rate trên mọi checkpoint. Agent output hành động cố định $\approx 0.46$ bất kể state. Cost không giảm qua 400k steps.
    \item \textbf{Discrete CQL V6 (10k$\to$50k)}: Cost giảm $\approx$64.5\% so với IQL (từ 726k xuống 258k) chỉ sau 50k steps. Đây là bằng chứng rõ ràng về hiệu quả của không gian hành động rời rạc.
    \item \textbf{Checkpoint 30k}: Là checkpoint duy nhất thoát khỏi miss rate 1.0 — 1/7 episode đã hoàn thành queue trước deadline. Đây là điểm đặc biệt cần phân tích thêm: khả năng mô hình đã học được chiến lược xả mạnh gần deadline, nhưng chưa tổng quát hóa được.
    \item Model đang tiếp tục train. Dự kiến miss rate sẽ giảm dần khi CQL penalty ổn định.
\end{itemize}
