...
\textbf{Kết luận chính:} Mô hình Discrete CQL kết hợp Safety Layer 20\% đạt hiệu năng vượt trội, tiết kiệm \textbf{11.1\% chi phí Gas} so với chiến lược Greedy truyền thống mà vẫn đảm bảo an toàn tuyệt đối (\textbf{0\% Deadline Miss}). Cần lưu ý rằng mức tiết kiệm này được tính toán dựa trên tổng chi phí của các giao dịch đã được gom nhóm (batching — tương tự như cơ chế L2), và hiệu quả này đạt được chủ yếu nhờ vào khả năng của mô hình trong việc lựa chọn chính xác thời điểm (timing) gửi giao dịch để tận dụng các vùng giá gas thấp trên mạng Ethereum. Mức tiết kiệm này tương đương với việc khai thác được 28.6\% tiềm năng tối đa của Oracle trong điều kiện không có thông tin tương lai.
...
\textbf{Nhận xét:}
\begin{itemize}
    \item \textbf{Win Rate = 91.7\%}: CQL thắng Greedy trong 222/242 episodes. Chỉ 20 episodes CQL thua, và mức thua rất nhỏ (tối đa -0.2\%).
    \item \textbf{Phân phối lệch phải:} Phần lớn episodes có mức tiết kiệm nhỏ (0-5\%), nhưng CQL khai thác cực mạnh ở các episodes có Gas biến động lớn (tiết kiệm tới 79.7\%). Đây chính xác là bản chất của bài toán Timing — lợi nhuận đến từ những cú ``bắt đáy'' hiếm hoi.
    \item \textbf{Tổng savings = 11.1\%:} Dù mean per-episode chỉ 1.4\%, tổng Gas Cost trung bình giảm 11.1\% vì CQL tiết kiệm nhiều nhất ở các episodes có chi phí cao nhất (hiệu ứng ``cứu chữa các ca nặng'').
\end{itemize}
...
# Output: Gas=65,525 | Miss=0.0% | vs Greedy=+11.1%
\end{lstlisting}

\textbf{Tài nguyên đóng băng (Frozen Assets):}
\begin{itemize}
    \item Model: \texttt{model\_160000.d3} (4.4MB)
    \item Data: \texttt{transitions\_v33\_L2\_Batching\_RAW.parquet} (1GB)
    \item Cả model và data đều là deterministic — không có yếu tố ngẫu nhiên ảnh hưởng kết quả.
\end{itemize}
