...
5    & $acc_t$                 & Acceleration: $m_t - m_{t-1}$ \\
...
\textbf{Thiết kế đặc biệt:} $b_t$ (Backlog Pressure) là mô hình tự hồi quy (Autoregressive - AR), đóng vai trò như một bộ nhớ ngắn hạn (short-term memory) để theo dõi áp lực tắc nghẽn chưa được giải phóng. Công thức cập nhật:
\begin{equation}
    b_t = \max(0, \; \rho\cdot b_{t-1} + \alpha_b\cdot p_t + \beta_b\cdot u_t)
\end{equation}
Các siêu tham số được chọn dựa trên đặc tính phân tích chuỗi thời gian (time-series analysis) của mạng Ethereum: $\rho=0.95$ (tương đương chu kỳ bán rã nửa vòng đời khoảng 14 block - phù hợp với thời gian kéo dài của các cú sốc MEV/NFT drop), $\alpha_b=0.3$ và $\beta_b=0.2$ dùng để cân bằng tác động giữa sự gia tăng gas sử dụng ($p_t$) và bất thường về số lượng giao dịch ($u_t$). Việc này giúp Agent cảm nhận được "sóng" tắc nghẽn trước khi giá gas thực sự chạm đỉnh.
...
\vspace{0.5em}\noindent\textbf{Bước 1 — Backward pass (tính value table):}
\begin{align}
...
\vspace{0.5em}\noindent\textbf{Bước 2 — Forward tracing (lấy quỹ đạo tối ưu):}
\begin{equation}
...
\begin{lstlisting}[style=PythonStyle, caption={Numba-Optimized Backward Induction (Source: oracle\_builder.py)}]
@njit  # Compile with Numba for lightning speed
def _compute_trajectory_njit(gas_prices, incoming_requests, Q_max, beta, alpha, ...):
    T = len(gas_prices)
    # Memory-safe grid size for Q (transaction-level precision)
    Q_bins = int(np.ceil(Q_max_real))
    V = np.full((T, Q_bins + 1), np.inf, dtype=np.float32)
    Policy = np.zeros((T, Q_bins + 1), dtype=np.uint8)

    # 1. Backward Induction (Value Table Propagation)
    for t in range(T - 2, -1, -1):
        for q_idx in range(Q_bins + 1):
            Q_real = float(q_idx)
            for b in range(n_bins):
                # Calculate execution volume based on discrete bin ratio
                n = int(min(np.floor(action_ratios[b] * Q_real), execution_capacity))
                remaining = max(0, int(Q_real) - n)
                # Next step recursive value lookup
                Q_next = remaining + float(incoming_requests[t+1])
                v_next = V[t+1, int(np.round(Q_next))]
                # Calculate costs: Execution + Urgency + Future Value
                cost = exec_cost(n, t) + urgency_penalty(remaining, t) + v_next
                if cost < best_cost:
                    best_cost = cost; best_bin = b
            V[t, q_idx] = best_cost
            Policy[t, q_idx] = best_bin
    return Policy
\end{lstlisting}

\subsection{Phân tích Dữ liệu (Data Description)}

Dữ liệu được thu thập từ \textbf{Ethereum Mainnet} \cite{ethereum2022eip1559} trong khoảng thời gian 8 tháng (2024--2025) thông qua API Etherscan và Infura. Sau quá trình Feature Engineering và Oracle labeling, tập dữ liệu cuối cùng có cấu trúc:
...
\hline
Tổng episodes & 1,206 (mỗi episode = 24h) \\
Tổng transitions & 8,616,371 \\
Tập Train (80\%) & 964 episodes — 6,884,703 transitions \\
Tập Test (20\%) & 242 episodes — 1,731,668 transitions \\
Số chiều state & 11 \\
...
Hình \ref{fig:state_dist} trình bày phân phối của 11 đặc trưng trạng thái. Đáng chú ý, $Q_t$ (Queue size) có phân phối lệch phải mạnh (right-skewed) với trung vị chỉ 396 nhưng giá trị cực đại lên tới 374,102, phản ánh sự xuất hiện không thường xuyên của các đợt tắc nghẽn lớn.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Images/01_state_distributions.png}
\caption{Phân phối 11 đặc trưng trạng thái trong tập dữ liệu. Đường cam nét đứt: giá trị trung bình ($\mu$); đường trắng chấm: trung vị ($m$). Các đặc trưng $g_t, g_{t-1}, g_{t-2}$ (giá gas) có phân phối bimodal phản ánh 2 chế độ thị trường (thanh khoản thấp và cao).}
\label{fig:state_dist}
\end{figure}

Hình \ref{fig:state_corr} cho thấy ma trận tương quan giữa các đặc trưng. Ba đặc trưng giá gas ($g_t, g_{t-1}, g_{t-2}$) có tương quan gần 1.0 với nhau — phản ánh tính liên tục của giá gas giữa các block liên tiếp. Đặc trưng $\bar{g}_t$ (gas reference) cũng tương quan cao ($r > 0.95$) với giá gas hiện tại, khẳng định vai trò là đường trung bình trượt (moving average).

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Images/02_state_correlation.png}
\caption{Ma trận tương quan Pearson giữa 11 đặc trưng trạng thái. Gas price ($g_t$) tương quan gần như hoàn hảo với $g_{t-1}, g_{t-2}$ và $\bar{g}_t$, nhưng gần như độc lập với $Q_t$ (queue size) — hàm ý rằng giá gas và lượng giao dịch là hai nguồn thông tin bổ sung cho nhau.}
\label{fig:state_corr}
\end{figure}

Hình \ref{fig:action_analysis} phân tích phân phối hành động trong tập dữ liệu hỗn hợp (70\% Oracle + 10\% Suboptimal + 20\% Behavior). Action 0 (giữ hàng) chiếm 56.7\% — phản ánh chiến lược "chờ đợi" của Oracle khi giá gas chưa đủ rẻ. Boxplot cho thấy Agent Oracle chủ yếu thực hiện Action 4 (xả 100\%) khi $Q_t$ nhỏ (queue gần hết) và chờ đợi (Action 0) khi $Q_t$ lớn.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Images/03_action_state_analysis.png}
\caption{Phân phối hành động (trên) và boxplot các đặc trưng theo hành động (dưới). Action 0 chiếm 56.7\% — Oracle ưu tiên chờ đợi khi giá gas cao.}
\label{fig:action_analysis}
\end{figure}

Hình \ref{fig:episode_snapshot} minh họa diễn biến của một episode điển hình (24 giờ, 265 blocks). Panel 1 cho thấy giá gas dao động trong khoảng [2.8, 5.5] Gwei. Panel 2 cho thấy Oracle điều phối queue giảm dần về 0 trước deadline. Panel 3 thể hiện pattern hành động: Oracle chủ yếu giữ hàng (Action 0, xanh dương) trong giai đoạn gas cao, và xả mạnh (Action 3-4, xanh lá/đỏ) khi gas giảm.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Images/04_episode_snapshot.png}
\caption{Snapshot một episode điển hình (ep\_id=316). Oracle chờ đợi khi gas cao (vùng trước block 150) và xả mạnh khi gas giảm (vùng sau block 200), giảm queue về 0 trước deadline.}
\label{fig:episode_snapshot}
\end{figure}
...
\vspace{0.5em}\noindent\textbf{Bước 1 — Feature Engineering (\texttt{build\_state\_action.py}):}
...
\vspace{0.5em}\noindent\textbf{Bước 2 — Discrete Oracle (\texttt{oracle\_builder.py}):}
...
\vspace{0.5em}\noindent\textbf{Bước 3 — Queue Recompute:}
...
\vspace{0.5em}\noindent\textbf{Bước 4 — Reward Calculation (\texttt{build\_reward\_episode.py}):}
Áp dụng công thức (\ref{eq:reward}) trên queue mới. Action bin ID được map sang ratio: \texttt{bins[action\_id]}.

\vspace{0.5em}\noindent\textbf{Bước 5 — Dataset Mix:}
\begin{itemize}
...
\begin{lstlisting}[style=PythonStyle, caption={Cài đặt môi trường Discrete (Source: enviroment.py)}]
    def step(self, action):
        # 1. Map Discrete Action ID -> Execution Ratio {0, 0.25, 0.5, 0.75, 1.0}
        ratio = self.config.action_bins[int(action)]
        # 2. Execution Volume Logic (Strict Integer Physics)
        ...
        # 3. Queue Dynamics (Arrivals from Mainnet history)
        if self.current_step + 1 < self.max_step:
            self.queue_size = remaining_q + self.arrival_arr[self.current_step + 1]
            self.time_to_deadline = self.t_deadline_arr[self.current_step + 1]
        # 4. Reward & State Transition
        reward = self._compute_reward(executed_volume, remaining_q)
...
\begin{lstlisting}[style=PythonStyle, caption={Certified Optimal Configuration (Source: config.py)}]
@dataclass(frozen=True)
class TransitionBuildConfig:
    # --- CANONICAL PHYSICS ---
    arrival_scale: float = 0.1          # 10% of mainnet traffic
    deadline_penalty: float = 20000.0   # Shaped penalty for exploration
    episode_hours: int = 24             
    # --- ECONOMICS & URGENCY ---
    urgency_alpha: float = 3.5          # Exponential steepness
    urgency_beta: float = 0.0005        # Optimal urgency weight
    action_bins: tuple = (0.0, 0.25, 0.5, 0.75, 1.0) # Discrete Action Space
\end{lstlisting}
...
\begin{table}[H]
\centering
\caption{Hyperparameters của Discrete CQL V6 (Model Chiến thắng)}
\begin{tabular}{lll}
\hline
...
Batch size & 1024 & Ổn định gradient với dataset 4.2M transitions \\
Discount $\gamma$ & 0.99 & Dense per-step reward, không cần giảm $\gamma$ \\
Conservative $\alpha$ & 1.0 & Bảo thủ vừa phải cho bài toán rủi ro bất đối xứng \\
Encoder & MLP [512, 512, ReLU] & Đủ sức biểu diễn cho state 11 chiều \\
Observation scaler & MinMax & d3rlpy tự normalize tại training time \\
Reward scaler & Standard ($\mu$=-0.063, $\sigma$=0.75) & Chuẩn hóa reward về $\mathcal{N}(0,1)$ \\
Training steps & 500,000 & Checkpoint tốt nhất tại step 160,000 \\
...
\begin{equation}
    Q_\theta(s, \cdot) = \text{Linear}_{512\to5}\;\circ\;\text{ReLU}\;\circ\;\text{Linear}_{512\to512}\;\circ\;\text{ReLU}\;\circ\;\text{Linear}_{11\to512}(s)
    \label{eq:qnet}
\end{equation}
...
Hình \ref{fig:training_curves} trình bày diễn biến 2 thành phần loss qua 500,000 steps huấn luyện:

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Images/05_training_curves.png}
\caption{Đường cong huấn luyện Discrete CQL V6. \textbf{Trên:} TD Loss tăng đều — Q-values đang phân tách về magnitude, thể hiện mạng đang học phân biệt hành động tốt/xấu. \textbf{Dưới:} Conservative Loss giảm đều từ 1.60 xuống 1.52 — Q-network học được cách hạ thấp giá trị của các hành động OOD so với in-distribution. Đường xanh lá đánh dấu checkpoint 160K (mô hình tốt nhất).}
\label{fig:training_curves}
\end{figure}

\textbf{Nhận xét quan trọng:}
\begin{itemize}
    \item \texttt{conservative\_loss} giảm liên tục và ổn định — xác nhận Q-network đang học phân biệt in-distribution (Oracle) actions với OOD actions.
    \item \texttt{td\_loss} tăng dần — đây là hành vi bình thường trong CQL vì conservative penalty đẩy Q-values phân tán ra xa, làm Bellman target $y$ cũng thay đổi theo.
    \item Checkpoint tốt nhất (160K) nằm ở vùng Conservative Loss bắt đầu giảm chậm lại (``elbow''), cho thấy mô hình đã học được phần lớn cấu trúc dữ liệu trước khi bắt đầu bão hòa.
\end{itemize}
...
\subsection{Phương pháp Đánh giá Chống Thiên kiến Chọn lọc (Hold-out Validation)}

Trong quá trình đánh giá các mô hình học máy, đặc biệt là dữ liệu chuỗi thời gian, việc sử dụng toàn bộ tập Test để chọn ra mô hình tốt nhất (Model Selection) sẽ dẫn đến \textbf{Thiên kiến Chọn lọc (Selection Bias)}. Điều này khiến mô hình bị overfit ngầm (Implicit Overfitting) vào tập Test, dẫn đến báo cáo hiệu năng thiếu chính xác so với thực tế.

Để loại bỏ hoàn toàn Selection Bias, tập Test được chia làm 2 giai đoạn độc lập (Hold-out Validation):
\begin{itemize}
    \item \textbf{Giai đoạn 1 (Validation Split - 121 episodes đầu):} Đánh giá toàn bộ các checkpoint của mô hình để chọn ra mô hình tốt nhất (\texttt{model\_40000.d3}).
    \item \textbf{Giai đoạn 2 (Blind Test Split - 121 episodes sau):} Khóa mọi quyết định và chỉ đánh giá \textbf{duy nhất} mô hình đã chọn trên tập dữ liệu hoàn toàn chưa từng được quan sát này. Con số thu được từ tập này là hiệu năng tổng quát hóa (Generalization Performance) thực sự.
\end{itemize}

Hơn nữa, để đảm bảo tiêu chuẩn khắt khe của hệ thống thực tế (Production), \textbf{Safety Layer} (ép xả 100\% giao dịch khi thời gian còn dưới 20\%) được tích hợp trực tiếp vào vòng lặp đánh giá của mô hình ở cả hai giai đoạn. Điều này ngăn việc ưu ái các mô hình "liều mạng" (tiết kiệm nhiều nhưng trượt deadline) và đảm bảo tìm ra chiến lược phối hợp hoàn hảo nhất với lớp an toàn.

\subsection{Bảng xếp hạng Hiệu năng (Leaderboard) - Tập Blind Test}

Bảng \ref{tab:final_leaderboard} trình bày kết quả đánh giá cuối cùng trên 121 episodes của tập Blind Test độc lập. Các chỉ số đánh giá bao gồm:
\begin{itemize}
...
\begin{table}[H]
\centering
\caption{Bảng Xếp Hạng Hiệu Năng Tối Ưu Gas (Tập Blind Test 121 episodes)}
\label{tab:final_leaderboard}
\begin{tabular}{llccc}
\hline
\textbf{\#} & \textbf{Chiến lược} & \textbf{Gas Cost $\downarrow$} & \textbf{Miss Rate $\downarrow$} & \textbf{vs Greedy} \\
\hline
1 & \textbf{Oracle (Hindsight DP)} & \textbf{29,217} & \textbf{0.0\%} & \textbf{-27.6\%} \\
2 & \textbf{CQL model\_40000.d3 + Safety 20\%} & \textbf{36,494} & \textbf{0.0\%} & \textbf{-9.5\%} \\
3 & Greedy (Baseline: Bán Ngay) & 40,331 & 0.0\% & --- \\
4 & Random (Baseline: Không có Safety Layer) & 40,631 & 81.0\% & +0.7\% \\
5 & Decision Transformer (DT) & $>100,000$ & 90-100\% & Thất bại \\
\hline
\end{tabular}
\end{table}

\textbf{Kết luận chính:} Mô hình Discrete CQL kết hợp Safety Layer 20\% đạt hiệu năng vượt trội, tiết kiệm vững chắc \textbf{9.5\% chi phí Gas} so với chiến lược Greedy truyền thống mà vẫn đảm bảo an toàn tuyệt đối (\textbf{0\% Deadline Miss}). Mức tiết kiệm này được tính toán dựa trên tổng chi phí của các giao dịch đã được gom nhóm (batching), và hiệu quả này đạt được chủ yếu nhờ khả năng căn chỉnh (timing) vùng giá gas thấp trên mạng Ethereum. So với mốc trần 27.6\% của Oracle, Agent đã khai thác được 34.4\% tiềm năng tối đa trong điều kiện hoàn toàn mù thông tin tương lai.

\subsection{Phân tích Nguyên nhân Thất bại của Thuật toán Tiêu chuẩn}

Trước khi đạt được kết quả vượt trội này, các thuật toán RL mặc định như Decision Transformer (DT) và CQL với siêu tham số gốc đều thất bại thảm hại. 

\subsubsection{Sự Sụp đổ của Decision Transformer (Majority Bias)}
Decision Transformer (DT) \cite{chen2021decision} bản chất là Conditional Behavioral Cloning, học bằng hàm mất mát Cross-Entropy. Trong tập dữ liệu chuyên gia (Oracle), để chờ giá Gas cực tiểu, \textbf{Action 0 (Giữ hàng) chiếm tới 69.6\%}. Do mất cân bằng lớp (Class Imbalance) quá lớn, DT đã chọn giải pháp cực đoan để tối thiểu hóa loss: luôn dự đoán Action 0. Kết quả là DT giữ hàng liên tục, gây ra Miss Rate lên đến 100\%. Trái lại, CQL dùng phương pháp quy hoạch động (Q-Learning) nên có khả năng đánh giá giá trị thực (Q-value) của từng hành động, không bị phụ thuộc hoàn toàn vào tần suất xuất hiện như DT.
...
\begin{lstlisting}[style=PythonStyle, caption={Algebraic Mirror Reward Implementation (Source: enviroment.py)}]
# 1. Efficiency Savings (ref_gas - current_gas)
efficiency_savings = executed_volume * (gas_ref - gas_t)
# 2. Fixed Gas Overhead (C_base = 21000)
overhead_cost = (C_base / 1e9) * gas_t * (1.0 if executed_volume > 0 else 0.0)
R_eff = (efficiency_savings - overhead_cost) / gas_scaling_factor
# 3. Exponential Urgency Penalty
urgency_penalty = beta * remaining_q * np.exp(alpha * (1.0 - time_ratio))
# 4. Final Aggregated Reward with scaling
total_reward = (R_eff - urgency_penalty - R_deadline) / reward_scale
\end{lstlisting}
...
\hline
\textbf{Kết quả vs Greedy} & Thua 30\%+ & \textbf{Thắng 9.5\%} & \\
\hline
\end{tabular}
\end{table}

Thành công của mô hình V32 đến từ kỹ thuật \textbf{Reward Shaping} \cite{ng1999policy}:
\begin{enumerate}
...
Việc đánh đổi một phần lợi nhuận (từ mức tiết kiệm nguyên bản $>24\%$ xuống 9.5\%) để kích hoạt Safety Layer là minh chứng cho nguyên lý \textbf{Safe Reinforcement Learning}, mang lại giải pháp cân bằng hoàn hảo giữa Khả năng Tối ưu (Optimality) và Độ tin cậy (Reliability) cho hệ thống Blockchain Relayer.

\subsection{Phân tích Chi tiết Từng Episode}

Hình \ref{fig:per_episode} cung cấp cái nhìn chi tiết về hiệu năng CQL trên từng episode riêng lẻ:

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Images/06_per_episode_comparison.png}
\caption{\textbf{Trái:} Scatter plot so sánh Gas Cost từng episode (CQL vs Greedy). Điểm xanh: CQL thắng; điểm đỏ: CQL thua. Hầu hết điểm nằm \textit{dưới} đường break-even. \textbf{Phải:} Phân phối mức tiết kiệm (\%). CQL tiết kiệm trung bình 1.4\% mỗi episode, với một số episodes đạt tới 79.7\%.}
\label{fig:per_episode}
\end{figure}

\textbf{Nhận xét:}
\begin{itemize}
    \item \textbf{Win Rate = 76.9\%}: Trên tập Blind Test, mô hình thắng Greedy trong phần lớn các trường hợp. Khi thất bại, mức thua lỗ được Safety Layer giới hạn khá an toàn (tối đa chỉ -4.5\%).
    \item \textbf{Phân phối lệch phải:} Phần lớn episodes có mức tiết kiệm nhỏ (trung vị 0.0\%), nhưng mô hình khai thác cực tốt ở các episodes có biến động lớn (mức tiết kiệm tối đa 42.2\%). Đây chính xác là bản chất của bài toán Timing — lợi nhuận đến từ những cú ``bắt đáy'' hiếm hoi.
    \item \textbf{Tổng savings = 9.5\%:} Dù trung vị tiết kiệm mỗi episode có thể thấp, tổng Gas Cost trung bình giảm 9.5\% vì CQL tiết kiệm rất lớn ở các episodes có chi phí cực đoan (hiệu ứng ``cứu chữa các ca nặng'').
\end{itemize}

\subsection{Tính Tái lập (Reproducibility)}

Toàn bộ kết quả trên có thể tái lập 100\% bằng script \texttt{reproduce\_sota.py}:

\begin{lstlisting}[style=PythonStyle, caption={Lệnh tái lập kết quả}]
# Chạy trên bất kỳ máy nào có Python 3.10+, d3rlpy, torch
python Final_Project/reproduce_sota.py
# Output: Gas=36,494 | Miss=0.0% | vs Greedy=+9.5%
\end{lstlisting}

\textbf{Tài nguyên đóng băng (Frozen Assets):}
Toàn bộ mã nguồn, mô hình đã huấn luyện và tập dữ liệu thực nghiệm được lưu trữ tập trung để phục vụ quá trình thẩm định và tái lập kết quả:
\begin{itemize}
    \item \textbf{Thư mục lưu trữ tài nguyên (Google Drive):} \url{https://drive.google.com/drive/folders/1P6jACyDDxfretWMDIuI3MN0kxa0hVDEr?usp=sharing}
    \item \textbf{Cấu trúc thư mục:} Bao gồm Model SOTA (\texttt{.d3}), Dataset tập Test (\texttt{.parquet}), Script chạy tái lập kết quả (\texttt{reproduce.py}), file quy định gói thư viện (requirements.txt) và File cấu hình chuẩn hóa (\texttt{.json}) cũng như các file log khi build data transition và train model CQL.
    \item Toàn bộ tài nguyên đều là tất định (deterministic), đảm bảo tính tái lập 100% khi chạy cùng script \texttt{reproduce.py}.
\end{itemize}
