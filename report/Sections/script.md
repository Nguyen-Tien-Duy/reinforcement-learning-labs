...
**Reward** ($r_t$) được thiết kế để tối ưu hóa đồng thời 3 mục tiêu:

$$r_t = \underbrace{-\frac{g_t}{\sigma}(C_{base}\mathbb{1}_{n_t>0} + C_{mar}n_t)}_{\text{Gas Cost}} - \underbrace{\frac{\beta}{\sigma} Q_{res} e^{\alpha(1-\tau_t)}}_{\text{Urgency}} - \underbrace{\frac{\lambda}{\sigma}\mathbb{1}_{Q_T>0}}_{\text{Deadline}}$$

Trong đó, các hằng số $\beta$ và $\lambda$ được chúng em hiệu chuẩn để cân bằng giữa mục tiêu tiết kiệm và độ an toàn tuyệt đối."
...
**Thứ hai**, Bất đối xứng rủi ro là rào cản lớn nhất. Với tỷ lệ phạt trễ deadline gấp 223 nghìn lần lợi nhuận tiết kiệm gas, Agent luôn có xu hướng chọn giải pháp an toàn nhất là xả hàng ngay. Chúng em đã phải tinh chỉnh Reward Shaping cực kỳ khéo léo để vượt qua định kiến này.

**Thứ ba**, hệ thống Lai AI + Safety Layer là chuẩn mực cho ứng dụng thực tế, cân bằng giữa tối ưu và độ tin cậy.

**Thứ tư**, về hướng phát triển, nhóm đề xuất 3 hướng nghiên cứu chuyên sâu: 
1. Chuyển sang không gian hành động liên tục kết hợp bộ giải MILP để tối ưu chính xác hơn.
2. Áp dụng Meta-Learning để hệ thống tự thích nghi với các giai đoạn thị trường khác nhau.
3. Và cuối cùng là tích hợp trực tiếp các ràng buộc an toàn vào quá trình học (Constrained RL) thay vì dùng Safety Layer cứng.

Em xin kết thúc phần trình bày tại đây. Cảm ơn Cô và các bạn đã chú ý lắng nghe. Em rất mong nhận được những câu hỏi và góp ý từ Hội đồng!"
