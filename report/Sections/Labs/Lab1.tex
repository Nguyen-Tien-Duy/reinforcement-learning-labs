...
\end{figure}
Ta thấy rằng reward luôn là -200 điều này có thể thấy agent luôn thua quá 200 bước - điều kiện để kết thúc một episode của MountainCar là xe đạt đến đỉnh núi hoặc hết thời gian (200 bước)
...
\begin{lstlisting}[style=PythonStyle, caption={Value Iteration Implementation}]
def value_iteration(P, R, gamma=0.99, theta=1e-6):
    ...
    while True:
        ...
        for s in range(NUM_STATES):
            Q_s = [R[s, a] + gamma * np.sum(P[s, a] * V) for a in range(NUM_ACTIONS)]
            best_val = np.max(Q_s)
...
Khác với Value Iteration lặp lại trên hàm giá trị, Policy Iteration chia quá trình học thành hai bước tuần tự. Điều này giúp thuật toán thường hội tụ sau rất ít vòng lặp lớn, mặc dù chi phí tính toán trong mỗi vòng lặp là cao hơn:

\textbf{Bước 1: Đánh giá chính sách (Policy Evaluation)}
Tính toán hàm giá trị $V^\pi(s)$ cho một chính sách cố định $\pi$ hiện tại mà không thực hiện việc chọn hành động mới (không có toán tử $\max$).
\begin{equation} 
    V_{k+1}(s) = R(s, \pi(s)) + \gamma \sum_{s' \in \mathcal{S}} P(s'|s,\pi(s)) V_k(s')
\end{equation}

\begin{lstlisting}[style=PythonStyle, caption={Policy Evaluation}]
def policy_evaluation(policy, P, R, gamma=0.99, theta=1e-6):
    ...
    return V
\end{lstlisting}
\textbf{Bước 2: Cải tiến chính sách (Policy Improvement)}
Dựa vào hàm giá trị đã được đánh giá ở Bước 1, cập nhật lại chính sách bằng cách chọn hành động đem lại kỳ vọng cao nhất cho mỗi trạng thái.
\begin{equation} 
    \pi_{new}(s) = \arg\max_{a \in \mathcal{A}} \left( R(s, a) + \gamma \sum_{s' \in \mathcal{S}} P(s'|s,a) V^\pi(s') \right)
\end{equation}

\begin{lstlisting}[style=PythonStyle, caption={Policy Improvement}]
def policy_iteration(P, R, gamma=0.99, theta=1e-6):
    ...
    return policy, V
\end{lstlisting}Quá trình lặp lại Bước 1 và Bước 2 sẽ dừng lại khi chính sách không còn thay đổi ($\pi_{new}(s) = \pi_{old}(s) \ \forall s \in \mathcal{S}$).
