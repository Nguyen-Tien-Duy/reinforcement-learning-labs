...
\begin{frame}{Bảng Xếp Hạng Hiệu Năng — Tập Blind Test (121 Episodes)}
  \begin{table}
    \centering
    \begin{tabular}{llccc}
      \toprule
      ...
      \midrule
      1 & \textbf{Oracle (Hindsight DP)} & \textbf{29,217} & \textbf{0.0\%} & $-$27.6\% \\
      \rowcolor{UTHblue!15}
      2 & \textbf{CQL + Safety 20\%} & \textbf{36,494} & \textbf{0.0\%} & \textbf{$-$9.5\%} \\
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{Kết quả chính (Blind Test)}
        Tiết kiệm \textbf{9.5\% Gas} so với Greedy\\
        Miss Rate = \textbf{0.0\%}\\
        Khai thác \textbf{34.4\%} tiềm năng Oracle
      \end{alertblock}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Tính Ổn Định}
        Khử hoàn toàn \textbf{Selection Bias} nhờ chia tập Validation/Test\\
        Đảm bảo an toàn \textbf{100\%}
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Phân tích Chi tiết Từng Episode}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{itemize}
        \item CQL khai thác rất tốt ở các episodes có Gas biến động lớn (max savings \textbf{42.2\%})
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{itemize}
        \item Khi CQL thua, mức thua được kiểm soát tốt (tối đa $-$4.5\%) $\Rightarrow$ \textbf{Risk thấp}
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Kiến trúc Hệ thống Lai: RL + Safety Layer}
  ...
  \vspace{0.5em}
  \centering
  \textit{Đánh đổi: $>24\%$ $\to$ 9.5\% savings, nhưng Miss Rate: trượt $\to$ \textbf{0.0\%}}
\end{frame}
...
\begin{frame}{Kết luận}
  \begin{enumerate}
    ...
    \item \textbf{Risk-Reward Asymmetry:}
    Tỷ lệ phạt/lợi nhuận 223.000 : 1 buộc Agent phải ưu tiên sự an toàn tuyệt đối.

    \item \textbf{Hệ thống Lai = Chuẩn mực thực tiễn:}
    AI 80\% + Safety 20\% $\Rightarrow$ Cân bằng Optimality \& Reliability

    \item \textbf{Hiệu năng thực tế vững chắc:}
    Tiết kiệm \textbf{9.5\%}, \textbf{0\% Miss}, Loại bỏ Selection Bias
  \end{enumerate}

  \vspace{1em}
  \begin{block}{Hướng phát triển}
    \begin{itemize}
      \item \textbf{Continuous Action + MILP Oracle:} Tối ưu hóa chính xác đến từng Wei.
      \item \textbf{Meta-Learning:} Tự động thích nghi với các biến động (Regime shift) của thị trường.
      \item \textbf{Constrained RL:} Tích hợp trực tiếp ràng buộc an toàn (Safety) vào hàm Loss.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Backup: Natural Experiment — V6 vs V32}
  \begin{table}
    ...
    \begin{tabular}{lccc}
      ...
      \midrule
      \textbf{vs Greedy} & Thua 30\%+ & \textbf{Thắng 9.5\%} & \\
      \bottomrule
    \end{tabular}
  \end{table}
